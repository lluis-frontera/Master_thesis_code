...
outdir <- paste0(main.dir, "/03_Results/Donor_variability/Permutations/", perm_num.tag, "/", perm_idx.tag, "/", opt$cell_level, "/", opt$cell_type, "/")

if(!file.exists(outdir)){dir.create(outdir, recursive = T, showWarnings=T)}
...
# 1.0 Get permuted metadata
perm.fn <- paste0(main.dir, "/03_Results/Permutations/", perm_num.tag, '/OneK1K/Age/',
...
## Filter genes with less than 20 reads across sum of expression of all cells
gene_counts <- rowSums(counts(sce_object))
selected_genes <- names(gene_counts[gene_counts >= 20])
sce_filtered <- sce_object[selected_genes, ]

## Get counts and metadata for the filtered dataframe
count <- SingleCellExperiment::counts(sce_filtered)
meta <- colData(sce_filtered)

## Filter donors with at least 20 cells
don_distr <- meta %>% as.data.frame() %>% group_by(assignment) %>% summarise(ncell=n()) # Get number of cells for each donor
...
print(paste("Cells included in the following analysis:", ncol(count)))
print(paste("Genes with more than 20 reads across cells (will be analyzed):", nrow(count)))
print("################### End report ###################")
cat("\n")
...
### 2.1 Estimate gene expression variance based Negative Binomial model
...
TMP <- lapply(1:length(bti), function(i){ # WITH SECOND FILTERING (REMOVES TOO MANY GENES)

   time0 <- Sys.time()
   bti0 <- as.character(bti[i]) # Get current donor
   cell_i <- as.character(meta[meta$assignment==bti[i], "bare_barcode_lane"]) # Get cell information for current donor
   size_all <-  median(colSums(count[, cell_i])) # Get median for expression counts of genes for current donor (for normalization)

   genes <- rownames(count) # Get all genes
   v <- rep(NA,nrow(count)) # Variance parameter
   names(v) <- genes # Assign gene_id as row names
   b <- rep(NA,nrow(count)) # Mean parameter
   names(b) <- genes # Assign gene_id as row names
   phi <- rep(NA,nrow(count)) # Dispersion parameter
   names(phi) <- genes # Assign gene_id as row names
   se.mu <- rep(NA,nrow(count)) # Standard error mean
   names(se.mu) <- genes # Assign gene_id as row names
   se.phi <- rep(NA,nrow(count)) # Standard error dispersion
   ...
   cell_size <- colSums(count_i)/size_all # Cell size

   # Apply filtering to get only genes in which there are more than 15 counts in at least 15 cells

   count_e <- count_i > 0
   non_zero_reads <- rowSums(count_e) # Number of cells with non-zero reads
   gene0 <- genes[(row_sum>15)&(non_zero_reads>15)] # Get only genes in which there are more than 15 counts in at least 15 cells
   print(paste("Genes with more than 15 reads in at least 15 cells in donor", i, "(second filtering):", length(gene0)))
   ...
   ## For each gene
   tmp <- mclapply(gene0, function(k){ # Put "gene0" if filtering is needed
...
se_mean.fn <- paste0(outdir, "SE_Mean_", perm_idx.tag, ".rds")
...
Sx.phi <- lapply(TMP, function(ii) ii[[5]]) # Extract standard error dispersion of each gene of each cell of each donor
...
se_dis.fn <- paste0(outdir, "SE_Dispersion_", perm_idx.tag, ".rds")
...
ngene.fn <- paste0(outdir, "Number_genes_", perm_idx.tag, ".rds")
...
Vx <- readRDS(paste0(outdir, "Variance_", perm_idx.tag, ".rds"))
Bx <-readRDS(paste0(outdir, "Mean_", perm_idx.tag, ".rds"))
Phx <-readRDS(paste0(outdir, "Dispersion_", perm_idx.tag, ".rds"))
Sx.mu <-readRDS(paste0(outdir, "SE_Mean_", perm_idx.tag, ".rds"))
Sx.phi <-readRDS(paste0(outdir, "SE_Dispersion_", perm_idx.tag, ".rds"))
...
Phx <- log2(Phx) # Get log2 of dispersion
Bx <- log2(Bx) # Get log2 of mean
PhxNew <- Phx # Save current dispersion

for (i in bti){ # For each batch...
  Phx0 <- Phx[,i] # Get dispersions
  Bx0 <- Bx[,i] # Get means
...
saveRDS(PhxNew2, file = paste0(outdir, "Dispersion_updated_", perm_idx.tag, ".rds"))
print(paste("Residual dispersion saved in", paste0(outdir, "Dispersion_updated_", perm_idx.tag, ".rds")))
...
Bx <-readRDS(paste0(outdir, "Mean_", perm_idx.tag, ".rds"))
Phx <-readRDS(paste0(outdir, "Dispersion_", perm_idx.tag, ".rds"))
PhxNew <-readRDS(paste0(outdir, "Dispersion_updated_", perm_idx.tag, ".rds"))
Sx.mu <-readRDS(paste0(outdir, "SE_Mean_", perm_idx.tag, ".rds"))
Sx.phi <-readRDS(paste0(outdir, "SE_Dispersion_", perm_idx.tag, ".rds"))
...
figfn_1 <- paste0(figfn, "1.1.Density_basic_par_", perm_idx.tag, ".rds")
...
lapply(seq(1, num_iterations), function(i) {
  ...
  figfn_4_donors <- paste0(figfn_4, "1.2.Density_SE_dispersion_filt_", perm_idx.tag, "_", start_col, "_", end_col, ".png")
  ...
  fig1 <- ggplot(dd3, aes(x=x,y=y))+
          geom_point() +
          geom_text(data=anno_df1, aes(x=2, y=6, label=corr2), parse=T, size=2)+
  ...
  figfn_5_donors <- paste0(figfn_5, "2.1.Mean_var_scatter_", perm_idx.tag, "_", start_col, "_", end_col, ".png")
  ...
  figfn_6_donors <- paste0(figfn_6, "2.2.Mean_dispersion_scatter_", perm_idx.tag, "_", start_col, "_", end_col, ".png")
  ...
  figfn_7_donors <- paste0(figfn_7, "2.3.Mean_newdispersion_scatter_", perm_idx.tag, "_", start_col, "_", end_col, ".png")

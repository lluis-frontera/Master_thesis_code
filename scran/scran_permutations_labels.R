...
# Categorize Age
colData(cell_md)$Age_cat <- ifelse(colData(cell_md)$Age<=40, 'Y',
                               ifelse(colData(cell_md)$Age>=60, 'O', 'M'))
colData(cell_md)$Age_cat_all <- ifelse(colData(cell_md)$Age<=40, 'Y', 'O')

# Get informative columns
rnames <- rownames(cell_md)
donor_vars <- c('assignment','date','Gender','Age', 'Age_cat', 'Age_cat_all')
donor_md <- colData(cell_md)[,donor_vars]

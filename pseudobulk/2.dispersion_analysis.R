...
if(startsWith(getwd(), path_local)){
  ...
  main.dir <- paste0(path_cluster, "/03_Results/Donor_variability/", opt$cell_level, "/", opt$cell_type, "/")
}

results_dir <- paste0("Results_", opt$phenotype, "/")
...
myDE <- function(y, gene, design_matrix, nna, threshold = 3) {
  
  # y: Vector of gene expression values
  # gene: Gene identifier
  # design_matrix: Design matrix for covariates
  # nna: Number of non-missing expression values across donors per gene
  # threshold: A threshold value for the number of samples (default is 8)
  ...
  if (class(y) != "try-error" && class(lm0) != "try-error") {
    ...
    nn <- names(b) # Get coefficient names
...
PhxNew <- readRDS(paste0(main.dir, "Dispersion_updated_5threshold_filtered.rds"))

# PhxNew <- PhxNew + 0.0001 # Add psuedocount to non NA values
# PhxNew[is.na(PhxNew)] <- 0.0001 # Add pseudocount to NA values

if(startsWith(getwd(), path_local)){
  sce_object <- readRDS(paste0("/home/lfronter/Downloads/scRNAseq/Yazar2022/sce_data_objects/", opt$cell_type, "_", opt$cell_level, "_sceraw.rds"))
} else {
...
covariates_vec <- unlist(strsplit(opt$covariates, split = ",")) # Get covariates vector
form_covs.string <- paste('~', opt$phenotype, "+", paste0(covariates_vec, collapse = " + "))
...
design_mat <- model.matrix(form_covs, metadata_donor) # Get pseudobulked design matrix
...
TMP <- mclapply(rn, function(ii){
  ...
  nna <- sum(!is.na(y))
  dd <- myDE(y, ii, design_mat, nna, threshold=3)
  dd
}, mc.cores=1) ### End loop by gene
TMP <- TMP[!sapply(TMP, is.null)] # Filter NULL values
TMP <- as.data.frame(do.call(rbind, TMP)) %>% mutate(celltype =  opt$cell_type)
...
opfn <- paste0(outdir, "1.DVG_results_5threshold_filtered.rds")
saveRDS(TMP, file = opfn)
print(paste("DVA results for", opt$cell_type, "saved in", opfn))
...
DVGs <- readRDS(paste0(outdir, "1.DVG_results_5threshold_filtered.rds")) 
DVGs <- DVGs %>% mutate(significance=ifelse(p_value_adj < 0.05, "sig", "not_sig"))
di <- DVGs %>% arrange(p_value_adj) # Sort by pvalue in ascending order
ngene <- nrow(DVGs) # Get number of genes 
di <- di %>% mutate(observed=-log10(p_value_adj), expected=-log10(ppoints(ngene))) # Get observed and expected p-values for qqplots
...
figfn <- paste0(outdir, "Figure1_Summary_Results_5threshold_filtered.pdf")
pdf(figfn, width=12, height=6)
print(plot_grid(fig1, fig2, ncol=2, labels="AUTO", label_fontface="plain"))
dev.off()

print(paste("Summary plots for", opt$cell_type, "saved in", figfn))